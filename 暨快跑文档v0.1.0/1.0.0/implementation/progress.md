...
## T09 记录（管理与治理，2026-09-25）

**新增文件**

- 公共模块：`jirun-domain/moderation-policy.js`（治理规则）、
  `moderation-repository.js`（契约）、`moderation-service.js`（治理服务）。
- 集合：`jr_reports`、`jr_appeals`（decisionId 唯一索引，拒绝重复申请）、
  `jr_config`（key 唯一），三者禁止客户端直接读写。
- 云对象：新建 `jirun-moderation`（用户侧提交举报／本人案件／复核申请），
  扩展 `jirun-admin`（审核决定、队列、账号限制、配置、概览）。
- 用户端：`services/moderation.js`、`pages/report/create.vue`、
  `pages/report/history.vue`；详情页加入举报入口；「我的」加入举报记录入口。
- 管理端：`services/jirun-admin.js`、`pages/jirun/` 下 6 个页面
  （overview／content／reports／appeals／users／config）；管理菜单新增「内容治理」目录及 6 项子菜单。
- 测试：`tests/unit/moderation-policy.test.cjs`（10 项）、
  `tests/unit/moderation-service.test.cjs`（34 项）。
- 预览工具链扩展：支持构建管理端（`preview:build:admin`／`preview:serve:admin`）。

**执行命令与真实结果**

| 命令 | 退出码 | 实际结果 |
|---|---|---|
| `npm test -- moderation-policy`（实现前） | 1 | `MODULE_NOT_FOUND` |
| `npm test -- moderation` | 0 | 44 项通过 |
| `npm test`（全部） | 0 | 197 项通过 |
| `npm run check:pages` | 0 | 用户端 32 页、管理端 64 页；菜单 44 条全部有效；11 个 `jr_*` 集合权限已逐一检查 |
| `npm run preview:build` / `preview:build:admin` | 0 | 两个工程均编译成功 |
| 浏览器实测（用户端举报页） | — | 举报对象与摘要、6 个原因选项、「处理期间保持原有可见性」与「举报不收费」说明 |
| 浏览器实测（用户端举报历史） | — | 三类筛选；一条「内容被下架／已处理／原因」并带「申请复核」，一条「举报信息／处理中」 |
| 浏览器实测（管理端 6 页） | — | 治理概览、内容审核、举报处理、复核处理、账号限制、平台配置均渲染成功 |

**管理端进入方式（预览）**

管理端登录依赖云端，无服务空间时会停在登录页；治理页面可通过整页加载带 hash 的
地址直接查看，例如 `http://127.0.0.1:5181/#/pages/jirun/overview`。初次加载后再用
路由跳转会被登录守卫拦回登录页，需重新整页加载。这只影响预览浏览，权限判定在服务端。

**实现中的取舍**

- 举报受理与下架严格分离：`submitReport` 只写记录，可见性不变；只有 `uphold`／`remove` 才下架。
- 审核必须指向当前待审版本，旧版本的审核结果不能发布用户后来修改的版本。
- 复核受理不改变可见性；复核通过仍需 `canRestore` 通过，作者已删除的内容一律拒绝。
- 决定与审计日志成对写入，记录目标版本、原因与操作者，作者可在本人案件里看到下架原因。
- 账号限制限制与解除都必须写明原因，且不存在自动封号或自动超时处罚。
- 配置键不允许出现付费能力开关（`isForbiddenConfigKey` 有 2 项测试覆盖）。
- 明确不提供管理员浏览他人私聊的能力，并用恒返回 false 的规则固化。
- 媒体审核返回明确的 `DEPENDENCY_UNAVAILABLE`，不假装通过。
- 预览占位改为属性拦截，因为管理端的 store 在启动阶段就访问数据库，
  不接管会在页面挂载前崩溃（早先只做「不存在时才注入」是不够的）。

**未完成**

- **云端治理仓储未实现**：`jirun-moderation/repository.js` 与前两个服务同样是有意留空
  （D-09）。队列游标、条件更新与唯一索引冲突需要真实服务空间才能验证。
- **真实权限验证未执行**：伪造管理员角色、跨用户查询举报、重复审核、删除后复核、
  媒体下架权限等 35 条真实云端验收见 `tests/cloud/moderation-cases.md`（M-01～M-35，均未执行）。
- **管理端菜单数据未验证**：菜单来自云端集合，预览中看不到侧边栏。
- **交互仍未验证**：执行环境浏览器无可见画面，仅验证了渲染与数据。
- 媒体与审核管道（T05）、私聊（T08）、资料与通知（T10）未开始。
